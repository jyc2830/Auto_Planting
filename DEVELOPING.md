...
2. Builds legacy, modern, and current jars
3. Creates `release/`
4. Copies only the final JARs into `release/`
...
| --- | --- | --- | --- |
| Legacy | `1.12.x ~ 1.16.5` | CraftBukkit, Spigot, Paper | Pre-1.17 compatibility bucket. |
| Modern | `1.17.x ~ 1.21.x` | CraftBukkit, Spigot, Paper | Shared non-Folia build. |
| Current | `26.1.x` | CraftBukkit, Spigot, Paper, Folia | Folia-enabled build. |
...
- Add a new `VersionAdapter` implementation under `src/main/java`
- Add runtime-specific logic only when a server family requires it
- Keep common behavior in shared code unless the API is missing on older versions
